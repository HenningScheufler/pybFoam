#!/usr/bin/env python3
"""
Clean generated .pyi stubs by replacing leaked C++ types with Python types.

Works with stubs generated by nanobind.stubgen (primary) or pybind11-stubgen.
"""
...
TYPE_REPLACEMENTS = {
    # ===================================================================
    # std:: C++ types that leak through nanobind stubgen
    # ===================================================================
    # std::vector<double, ...> -> Sequence[float]
    r'"std::vector<double,\s*std::allocator<double>\s*>"': "Sequence[float]",
    # std::tuple<int, double> -> tuple[int, float]
    r'"std::tuple<int,\s*double>"': "tuple[int, float]",
    # std::tuple<double, double> -> tuple[float, float]
    r'"std::tuple<double,\s*double>"': "tuple[float, float]",
    # Generic fallback for any remaining std::tuple
    r'"std::tuple<([^"]+)>"': r'"tuple[\1]"',
    # ===================================================================
    ...
    r"\b__all__: list\b": "__all__: list[str]",
}
...
def main():
    ...
    for stub_file in stub_files:
        ...
        if "typing.Any" in content and "import typing" not in content:
            # Add after the first line (module docstring or import)
            lines = content.split("\n")
            insert_idx = 0
