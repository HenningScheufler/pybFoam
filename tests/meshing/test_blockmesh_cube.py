...
@pytest.fixture
def temp_case_native(tmp_path, cube_case):
...
@pytest.fixture
def temp_case_python(tmp_path, cube_case):
...
def run_native_blockmesh(case_path: Path, log_file: Path):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"blockMesh failed with return code {result.returncode}. See log: {log_file}")
...
def test_blockmesh_comparison(temp_case_native, temp_case_python, tmp_path):
    ...
    assert native_stats['mesh_stats']['internal_faces'] == python_stats['mesh_stats']['internal_faces'], \
