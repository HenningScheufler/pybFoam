...
@pytest.fixture
def temp_case_native(tmp_path, motorbike_case):
...
def test_snappy_phase_comparison(
    ...
    run_surface_feature_extract(temp_case_native, log_dir / f"{test_name}_native_surfaceFeatureExtract.log")
    
    run_blockmesh(temp_case_python, log_dir / f"{test_name}_python_blockmesh.log")
    run_surface_feature_extract(temp_case_python, log_dir / f"{test_name}_python_surfaceFeatureExtract.log")
    ...
    if errors:
        pytest.fail(f"Mesh statistics do not match:\n" + "\n".join(errors))
