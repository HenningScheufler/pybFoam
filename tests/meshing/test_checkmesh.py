...
def parse_checkmesh_output(cmd_output: str) -> Any:
    ...
    if (
        pair := extract_float_pair(
            r"Face flatness.*?min = ([-\d.e+\-]+)\s+average = ([-\d.e+\-]+)", cmd_output
        )
    ) is not None:
    ...
    if (
        pair := extract_float_pair(
            r"Face interpolation weight.*?minimum:\s+([-\d.e+\-]+)\s+average:\s+([-\d.e+\-]+)",
            cmd_output,
        )
    ) is not None:
...
def test_checkmesh_all_options(cube_case: Path, tmp_path: Path) -> None:
    ...
    assert result["geometry"]["min_volume"] == pytest.approx(
        parsed["geometry"]["min_volume"], rel=1e-6
    )
    assert result["geometry"]["max_volume"] == pytest.approx(
        parsed["geometry"]["max_volume"], rel=1e-6
    )
    assert result["geometry"]["total_volume"] == pytest.approx(
        parsed["geometry"]["total_volume"], rel=1e-6
    )
    assert result["geometry"]["min_face_area"] == pytest.approx(
        parsed["geometry"]["min_face_area"], rel=1e-6
    )
    assert result["geometry"]["max_face_area"] == pytest.approx(
        parsed["geometry"]["max_face_area"], rel=1e-6
    )
    assert result["geometry"]["min_edge_length"] == pytest.approx(
        parsed["geometry"]["min_edge_length"], rel=1e-6
    )
    assert result["geometry"]["max_edge_length"] == pytest.approx(
        parsed["geometry"]["max_edge_length"], rel=1e-6
    )
