...
def modify_snappy_dict(case_path: Path, castellated: bool = True, snap: bool = True, layers: bool = False):
    """
    ...
    content = content.replace('castellatedMesh true;', f'castellatedMesh {str(castellated).lower()};')
    content = content.replace('castellatedMesh false;', f'castellatedMesh {str(castellated).lower()};')
...
def run_blockmesh(case_path: Path, log_file=None):
...
def run_surface_feature_extract(case_path: Path, log_file=None):
...
def run_native_snappy(case_path: Path, log_file=None):
...
def run_python_snappy(case_path: Path, time: pyb.Time):
    ...
    # Run snappyHexMesh
    meshing.generate_snappy_hex_mesh(
        mesh=mesh,
        dict=snappy_dict,
        overwrite=True,
        verbose=False
...
def compare_mesh_stats(native_stats: Dict[str, Any], python_stats: Dict[str, Any], tolerance: float = 1e-6):
    """
