...
from pybFoam.sampling import (
    sampledSurface,
    interpolationScalar, interpolationVector,
    sampleOnFacesScalar, sampleOnFacesVector,
    sampleOnPointsScalar,
    SampledPlaneConfig, SampledPatchConfig, SampledCuttingPlaneConfig, SampledIsoSurfaceConfig
)
import numpy as np
import os
...
def test_sampledPlane(change_test_dir):
    ...
    plane_model = SampledPlaneConfig(
        point=[0.5, 0.5, 0.0],
        normal=[1.0, 0.0, 0.0]
    )
...
def test_sampledPatch(change_test_dir):
...
def test_sampledIsoSurface(change_test_dir):
    ...
    iso_model = SampledIsoSurfaceConfig(
        isoField="p_rgh",
        isoValue=iso_value
    )
...
def test_interpolation_scalar_schemes(change_test_dir, scheme):
...
@pytest.mark.parametrize("scheme", ["cell", "cellPoint", "cellPointFace"])
def test_interpolation_vector_schemes(change_test_dir, scheme):
...
def test_interpolation_on_points_scalar(change_test_dir):
...
def test_interpolation_on_patch(change_test_dir):
...
def test_interpolation_scheme_comparison(change_test_dir):
...
# def test_sample_scalar_on_plane_faces(mesh, fields):
#     """Test sampling scalar field onto plane surface faces."""
#     p_rgh = fields["p_rgh"]
    
#     # Create a plane surface
#     plane_dict = dictionary()
#     plane_dict.set("type", Word("plane"))
#     plane_dict.set("point", vector(0.5, 0.5, 0.0))
#     plane_dict.set("normal", vector(1.0, 0.0, 0.0))
    
#     plane = sampledSurface.New(Word("samplePlane"), mesh, plane_dict)
#     plane.update()
    
#     # Create interpolation scheme
#     interp = interpolationScalar.New(Word("cellPoint"), p_rgh)
    
#     # Sample field onto surface
#     sampled_field = sampleOnFacesScalar(plane, interp)
    
#     assert sampled_field is not None
#     assert len(sampled_field) > 0
#     assert len(sampled_field) == len(plane.magSf())
    
#     # Check that sampled values are reasonable (not NaN or Inf)
#     sampled_array = np.asarray(sampled_field)
#     assert np.all(np.isfinite(sampled_array))


# def test_sample_vector_on_plane_faces(mesh, fields):
#     """Test sampling vector field onto plane surface faces."""
#     U = fields["U"]
...
#     plane_dict.set("type", Word("plane"))
#     plane_dict.set("point", vector(0.5, 0.5, 0.0))
#     plane_dict.set("normal", vector(0.0, 1.0, 0.0))
...
#     # Sample field onto surface
#     sampled_field = sampleOnFacesVector(plane, interp)
...
#     sampled_array = np.asarray(sampled_field)
#     assert sampled_array.shape == (len(sampled_field), 3)
#     assert np.all(np.isfinite(sampled_array))


# def test_sample_scalar_on_plane_points(mesh, fields):
#     """Test interpolating scalar field onto plane surface points."""
#     p_rgh = fields["p_rgh"]
...
#     # Interpolate field onto surface points
#     sampled_field = sampleOnPointsScalar(plane, interp)
    
#     assert sampled_field is not None
#     assert len(sampled_field) > 0
#     assert len(sampled_field) == len(plane.points())
...
# def test_sample_vector_on_plane_points(mesh, fields):
#     """Test interpolating vector field onto plane surface points."""
#     U = fields["U"]
    
#     # Create a plane surface
#     plane_dict = dictionary()
#     plane_dict.set("type", Word("plane"))
#     plane_dict.set("point", vector(0.3, 0.3, 0.0))
#     plane_dict.set("normal", vector(0.0, 0.0, 1.0))
...
#     # Interpolate field onto surface points
#     sampled_field = sampleOnPointsVector(plane, interp)
...
# def test_sample_fields_on_patch(mesh, fields):
#     """Test sampling fields on a patch surface."""
#     p_rgh = fields["p_rgh"]
#     U = fields["U"]
...
#     patches = wordList()
#     patches.append(Word("leftWall"))
#     patch_dict.set("patches", patches)
    
#     patch_surface = sampledSurface.New(Word("leftWallSurface"), mesh, patch_dict)
#     patch_surface.update()
    
#     # Sample scalar field
#     interp_scalar = interpolationScalar.New(Word("cell"), p_rgh)
#     sampled_p = sampleOnFacesScalar(patch_surface, interp_scalar)
    
#     assert len(sampled_p) == len(patch_surface.magSf())
#     assert np.all(np.isfinite(np.asarray(sampled_p)))
    
#     # Sample vector field
#     interp_vector = interpolationVector.New(Word("cell"), U)
#     sampled_U = sampleOnFacesVector(patch_surface, interp_vector)
    
#     assert len(sampled_U) == len(patch_surface.magSf())
#     assert np.all(np.isfinite(np.asarray(sampled_U)))


# def test_multiple_planes_different_orientations(mesh, fields):
#     """Test creating and sampling on multiple planes with different orientations."""
#     p_rgh = fields["p_rgh"]
...
#         plane_dict = dictionary()
#         plane_dict.set("type", Word("plane"))
#         plane_dict.set("point", vector(0.5, 0.5, 0.005))
#         plane_dict.set("normal", normal)
        
#         plane = sampledSurface.New(Word(f"plane_{idx}"), mesh, plane_dict)
#         plane.update()
...
#         assert len(sampled) > 0
#         assert len(sampled) == len(plane.magSf())


# def test_sampled_field_numpy_conversion(mesh, fields):
#     """Test that sampled fields can be converted to NumPy arrays and manipulated."""
#     p_rgh = fields["p_rgh"]
#     U = fields["U"]
...
#     # Sample scalar field
#     interp_scalar = interpolationScalar.New(Word("cellPoint"), p_rgh)
#     sampled_scalar = sampleOnFacesScalar(plane, interp_scalar)
    
#     scalar_array = np.asarray(sampled_scalar)
#     assert scalar_array.dtype == np.float64
#     assert scalar_array.shape == (len(sampled_scalar),)
    
#     # Sample vector field
#     interp_vector = interpolationVector.New(Word("cellPoint"), U)
#     sampled_vector = sampleOnFacesVector(plane, interp_vector)
    
#     vector_array = np.asarray(sampled_vector)
#     assert vector_array.dtype == np.float64
#     assert vector_array.shape == (len(sampled_vector), 3)
    
#     # Test that we can manipulate the arrays
#     scalar_mean = np.mean(scalar_array)
#     vector_magnitude = np.linalg.norm(vector_array, axis=1)
    
#     assert np.isfinite(scalar_mean)
#     assert np.all(np.isfinite(vector_magnitude))


# def test_different_interpolation_schemes_comparison(mesh, fields):
#     """Test and compare different interpolation schemes."""
#     p_rgh = fields["p_rgh"]
...
#         interp = interpolationScalar.New(Word(scheme), p_rgh)
#         sampled = sampleOnFacesScalar(plane, interp)
#         sampled_results[scheme] = np.asarray(sampled)
    
#     # All should have same size
#     for scheme, result in sampled_results.items():
#         assert len(result) == len(plane.magSf())
#         assert np.all(np.isfinite(result))
