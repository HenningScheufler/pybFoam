...
from pybFoam.sampling import (
    sampledSet, meshSearch,
    interpolationScalar, interpolationVector,
    sampleSetScalar, sampleSetVector
)
...
def test_meshSearch_creation(change_test_dir):
...
def test_sampledSet_uniform_line(change_test_dir):
...
def test_sampledSet_uniform_axes(change_test_dir, axis_name):
...
def test_sampledSet_cloud(change_test_dir):
...
def test_sampledSet_polyLine(change_test_dir):
...
def test_sampledSet_circle(change_test_dir):
...
def test_sample_scalar_field_on_uniform_set(change_test_dir):
    ...
    config = UniformSetConfig(
        axis="x",
        start=[0.05, 0.292, 0.005],
        end=[0.55, 0.292, 0.005],
        nPoints=30
    )
    set_dict = config.to_foam_dict()
...
def test_sample_vector_field_on_uniform_set(change_test_dir):
...
def test_sample_on_cloud_set(change_test_dir):
...
def test_multiple_sampling_schemes(change_test_dir, scheme):
...
def test_sampledSet_distance_calculation(change_test_dir):
...
def test_coordSet_interface(change_test_dir):
...
def test_edge_cases_outside_mesh(change_test_dir):
    ...
    cells = line.cells()
    invalid_cells = sum(1 for c in cells if c < 0)
...
def test_sample_multiple_fields_on_same_set(change_test_dir):
